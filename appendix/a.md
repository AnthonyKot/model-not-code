...
Chapter 7 priced a generated token and found three levers: a cache of keys and values so a decode step computes one row instead of a square, fewer bytes per weight so the memory-bound step reads less, and a batch so each read of the weights serves more customers. It worked the cache on a sixteen-dimensional generator, the mapping on a block of eight weights by hand, and batching on a bare matrix product. This appendix names the packaged form of the same three levers, a serving engine, and the settings that correspond to each; then two ways to put that engine on a rented GPU, a serverless endpoint and a deployed function with a persistent volume; and it ends by reading chapter 7's week 37 report from the engine's side. Nothing here changes the arithmetic of chapter 7. It says which knob the arithmetic is attached to.

## What the engine packages

The engine, vLLM, introduces itself with a feature list, as of the recording: serving throughput, paged attention for key and value memory, continuous batching of incoming requests, CUDA graphs, quantisation, optimised attention kernels, speculative decoding and chunked prefill. Read against chapter 7 the list is the three levers, one scheduler, one lever the chapter never touched, and two kernel-level items outside its arithmetic.
...
| CUDA graphs, attention kernels | not a lever in chapter 7's sense | the attention kernel removes excess reads and writes inside the attention step; CUDA graphs are named on the list and not explained by the sources here; the bytes of weights read per step do not change |
...
Two of those fields are chapter 7's numbers in another name. `max_tokens` caps the decode steps a request may cost, an upper bound where the week 37 report's 120 is a measured average; a limit of 100 cut a recorded answer off mid-sentence. `temperature` scales the writer's next-token scores before the softmax at sampling time: two recorded runs at 0.1 and 1.0 gave a narrower and a broader answer to the same question, and the section on the week 37 report says what it costs speculative decoding.
...
What a cold start is made of, chapter 7's source on serving spells out: getting a GPU, loading the container image, loading the weights, starting the engine, and for engines that compile, the compilation. The weight-loading term is bytes over bandwidth, which is why chapter 7's four-bit lever shows up here a second time. The week 37 generator's 7.0 GB of weights at two bytes each is 1.75 GB at half a byte, and the load takes a quarter of the time on the same link. The remedy the console offers, an active worker, is the price of the empty machine chapter 7's batching section ends on: a replica that is never cold costs its card every hour, requests or none.
...
| GPU memory: weights 7.0 GB + caches 1.2 GB of 24.0 GB | the weights as loaded, at two bytes each here, and the pages the cache manager has filled; chapter 7 worked 0.353 GB per request from the cache figure and the batch line above |

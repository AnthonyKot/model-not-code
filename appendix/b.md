...
Chapter 6's loop, rewritten against that API, keeps its three checks in their three places. The scripted writer becomes one request with `tools=tools`; the writer's request becomes `message.tool_calls`; the `messages.append` of a result becomes a `tool` message with the identifier. The schema check compares the requested name and its parsed arguments against the tools offered and their full parameter lists, as chapter 6's does; the confirmation check refuses a write the customer has not confirmed in the interface; the dispatch table maps a name to a function and nothing is ever evaluated from text. Chapter 6's figure names a fourth item between them, the permissions of the identity that runs the tool, which the provider's format does not touch.
...
def run(question, max_calls=5):
    ...
    for _ in range(max_calls):
        ...
        for call in message.tool_calls:                                         # several requests in one reply
            ...
            if spec is None or set(params) != set(spec["parameters"]["properties"]):        # 1. schema
...
The recordings also name what changes with the writer. The recorded small local model "gets quite confused with tool calling", and a larger open model does a decent job; the format is learned, and a model that saw little of it emits it badly. Chapter 6's coupon case reads the same against a real API. The seller's line in the kettle's sheet comes back inside a `tool` result; if the writer turns it into an `apply_coupon` request, the request arrives as a tool call like any other, and the second check refuses it because nobody confirmed that call in the interface. That the writer's final sentence can still say the kettle is free is chapter 6's finding, not the API's: the checks govern the action, and the sentence is governed by chapter 6's fourth line, a check of the answer against the tool results, and by its judge.

...
- **`correlated_checkpoints`** builds item-level outcomes with `torch.rand(...) < p` comparisons. Every checkpoint starts from one shared answer sheet and redraws a fraction `churn` of its items; churn 1.0 is the independent case, churn 0.02 is two neighbouring checkpoints that disagree on about one item in fifty.

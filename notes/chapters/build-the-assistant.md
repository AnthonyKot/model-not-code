...
### Cross-read repairs — 2026-09-22 (pair reviews 5+6 and 6+7)

Two reviewers reported that the standard-error example "on the lab's twelve golden questions, recall@3 of 0.833 against 0.917" quotes a pair no chunking produces (the four score 0.917, 0.583, 0.667, 0.500; the lab's Part 3 computes the 0.833/0.917 pair as an illustration and prints 0.134 and 0.027 for it). Applied: the sentence now says the pair is the lab's illustration and lists the four real values; "the two chunkings are indistinguishable" → "the two values"; the lab's code and printed lines are unchanged. Also: the hand trace's first search query now matches the lab's case A ("boil water fast", not "steel kettle"); the opening no longer says the first wrong answer happened "because" retrieval ranked the chunk fifth, but that retrieval ranked it fifth and the writer then attributed an unsupported figure, which is what the lab's T1 discussion says; one sentence after the SE formula says it is the conservative bound and chapter 5's per-item differencing is the sharper test on the same questions; the bridge says "3.4 requests decoding at once", chapter 7's name for the line. Not applied: a section for the writer stage (the reviewer's minor finding 9), and the note that chapter 4's writer could not emit JSON requests (the chapter already says a scripted writer stands in). Path 2382, folds 24.0%.

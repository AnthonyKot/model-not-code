...
### Cross-read repairs — 2026-09-22 (pair review 7+8)

Reviewer reported: the opening's "each was measured against nothing but itself" ignored chapter 2's never-flag baseline; "a product's lines" where the lab ranks chunks; the encoder "trained as chapter 6 trained it" reports 0.615 / 0.583 where chapter 6 prints 0.589 / 0.667 for the same chunking, because chapter 8 also trains on the six click-log pairs, said only after the table; "at chapter 7's price per token" pointed at a price chapter 7 did not state; the lucky-hit explanation said the lamp's chunk contains *the* (it contains *for*; *the* is in the watch's chunk only; the rank-2 result is three one-word hits, lamp, watch, keyboard, tie-broken by chunk order); "57.62 against 58.68" quoted a value the ladder folds into a range; "the lab's reader case" is production vocabulary. Applied: "apart from chapter 2's never-flag baseline none was measured against a rule somebody could have written instead"; "the catalogue's chunks"; "plus the six click-log pairs below, so its scores differ from chapter 6's" before the table; "about 0.0003 for the 120-token answer alone at chapter 7's report prices" (chapter 7 now derives it, receipt sac-50); the lucky hit re-explained from the data (verified by re-running the lab's chunking: *for* in chunks 2 and 10, *the* in chunk 8 only); "two rows inside the table's last frontier range"; "the lab's closing decision, \"Your call\"" twice. Five clauses shaved to pay for it. No printed number changed. Path 2720 (20 over the guideline), folds 22.8%.

...
### Cross-read repairs — 2026-09-22 (pair review 3+4)

Reviewer reported: the lab's writer is not chapter 3's (from random weights on 48 two-field demonstrations, no adapter) although the chapter said "chapter 3's block" and "chapter 3's demonstrations"; "fine-tuned writer" named a from-scratch model after chapter 3 defined a fine-tune as an update of pretrained weights; the reference copy is the full second copy chapter 3 said the shop cannot afford, with no word on where PPO sits at that size; the opening's "judged by the preferences the scorer was trained on" conflated the labels with the hidden scorer that 0.33 → −0.20 belong to; "the lab's 0.770 is that ceiling" against a computed 0.759. Applied: the lab paragraph says the writer is chapter 1's block with a feed-forward layer, trained from random weights, and not chapter 3's; "demonstration-trained writer" everywhere (figure aria-label, SVG label "trained writer", figcaption, results-table row, reference paragraph, hidden-score paragraph); one sentence in the reference paragraph saying that at chapter 3's size the tuned weights are an adapter on a frozen base and the reference is that base with the adapter as it was, so no second copy is made; the opening now names the hidden scorer that stands in for the people, the one the labels were sampled from; "sits at that ceiling, within the noise of 100 pairs". The lab's `Writer` docstring and walk-through now credit chapter 1's block with the output projection (workspace `exercise.py` synced; a docstring, so `run.log` is unchanged). To hold the path near 2,700: the hosted-services paragraph compressed into one sentence appended to "Where it stops", and four short clauses cut. Path 2726, folds 25.0% (26 over the 2,700 guideline; the fold share is at its ceiling, so nothing more was folded).

### Cross-read repairs, round two — 2026-09-22 (pair review 4+5)

The bridge said "a thumbs up on an answer that was shown" and "a silent drift"; chapter 4 has pairwise preferences, and chapter 5 reserves "drift" for the gradual case it says it did not simulate. Now "a preference label on answers that were shown" and "a silent change". The lab paragraph says the printed run labels write β as KL. Path 2732.

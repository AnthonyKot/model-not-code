...
export const chapters = [
  ...
  CH(4, "train-from-reward", "Train From Reward",
    ...
    [C("3725442", "02-05", "02-06", "02-08", "04-01", "05-03", "05-08", "06-03", "09-03", "09-04", "11-05"), C("4635836", "11-02", "12-01", "12-06", "13-01"), C("6100015", "06-22"),
     B("lapan-drl"), P("arXiv:1707.06347", "3, 5"), P("arXiv:2203.02155", "3.5, pp. 8–9")],
    { status: "published",
      caution: "The corridor, the answer writer, the hidden scorer that stands in for people, and the word-sum reward model are synthetic and tiny; the reward hack is built into that reward model's form. Hosted fine-tuning job types are as of the recording they come from; PyTorch behaviour as of 2.14." }),
  CH(5, "keep-it-right-after-launch", "Keep It Right After Launch",
    "The photo classifier after launch: what to keep so you can roll back, what reviewers' labels cannot tell you, an alarm that notices changed photos, and the evidence a retrained model needs before it replaces the live one.",
    [],
    [C("4735368", "18-02"), C("5004958", "08-01", "08-02"), C("6100015", "04-18", "06-08"),
     B("burkov-mle", "pp. 77, 139–140, 235–236, 249, 260"), B("huyen-dmls", "pp. 318–319, 323–325, 331–336"), B("geron-pytorch", "pp. 129, 131, 133, 728, 733"),
     B("ml-system-design", "§13.1, §14.4.4"), B("kubeflow-cml", "pp. 283–284, 289–290"), B("openshift-mlops", "pp. 155, 171–173"),
     P("https://github.com/pytorch/pytorch/blob/v2.14.0/torch/serialization.py"), P("https://github.com/pytorch/pytorch/blob/v2.14.0/torch/nn/modules/module.py")],
    { status: "published",
      caution: "Every example is synthetic: 12 x 12 photos, a banner and folding knives introduced on purpose, invented prices for a false flag, a missed blade and an audited listing. PyTorch behaviour as of 2.14; the ONNX description is as of the recording it comes from." }),
  CH(6, "build-the-assistant", "Build the Assistant",
    "The assistant answered wrongly: score retrieval on a golden set, see what a chunk boundary cuts off, trace the loop that runs the tools, refuse an instruction that arrived inside a listing, and tell which stage failed.",
    [],
  ...
  CH(7, "serve-the-assistant-cheaply", "Serve the Assistant Cheaply",
    "Where a generated token's cost goes and which lever moves it: keep the keys and values, read four bits per weight, batch the tokens, and read a serving report before buying a bigger card.",
    [],
  ...
  CH(8, "did-the-shop-need-a-model", "Did the Shop Need a Model?",
    "The ledger a model has to beat: a rule and the encoder on the same questions, someone else's ladder of models read with its error bars, and chapter 2's classifier against the rule the shop could have used instead.",
    [],
